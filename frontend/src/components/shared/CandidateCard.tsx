...
import { Bot, X, RotateCw, Info, MapPin, ShieldCheck } from 'lucide-react';
import { useLang } from '../../i18n/LangContext';
import type { Candidate, ReadinessConfig, ScoreDimension } from '../../types';
import { ReadinessDonut, ScoreBar, AIStatusBadge } from './Bits';
import { DEST_PHOTO } from '../../lib/images';
...
export default function CandidateCard({ candidate, config, startFlipped }: { candidate: Candidate; config: ReadinessConfig; startFlipped?: boolean }) {
        ...
        <div className={clsx('flip-inner w-full h-full', flipped && 'flipped')}>
          ...
          <div className={clsx('flip-face card overflow-hidden h-full flex flex-col border-t-4', stage.accent)}>
            <div className="relative h-40 bg-gradient-to-br from-ow-navy to-ow-blue flex-shrink-0">
              <img src={candidate.photo} alt={candidate.name} className="absolute inset-0 w-full h-full object-cover opacity-90" loading="lazy" />
              ...
              <div className="absolute bottom-3 left-4 right-4">
                ...
                <div className="flex items-center gap-1 text-white/80 text-xs mt-0.5"><MapPin className="w-3 h-3" /> {candidate.origin_city}</div>
              </div>
            </div>
            ...
            <div className="p-4 flex-1 flex flex-col">
              ...
              <div className="mt-auto flex items-center justify-between">
                ...
                <button onClick={() => setFlipped(true)} className="flex items-center gap-1.5 text-xs font-semibold text-ow-blue hover:text-ow-blue-dark transition-colors">
          ...
          <div className="flip-face flip-back card overflow-hidden h-full flex flex-col">
            ...
            <div className="p-4 flex-1 overflow-y-auto">
              ...
              <div className="mb-3">
                {Object.entries(p.score_breakdown).map(([key]) => {
                  const dim = dimByKey(key);
                  return <ScoreBar key={key} label={tl(dim.label_pt, dim.label_en)} score={p.score_breakdown[key]} onClick={() => setExplain(dim)} />;
                })}
              </div>
